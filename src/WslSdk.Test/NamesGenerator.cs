...
namespace WslSdk.Test
{
    internal static class NamesGenerator
    {
        ...
        {
			begin:
			var name = $"{Left[rand.Next(0, Left.Length - 1)]}_{Right[rand.Next(0, Right.Length - 1)]}";

			if (string.Equals(name, "boring_wozniak" /* Steve Wozniak is not boring */, StringComparison.OrdinalIgnoreCase))
				goto begin;

			if (retry > 0)
				name = $"{name}{rand.Next(0, 10)}";
